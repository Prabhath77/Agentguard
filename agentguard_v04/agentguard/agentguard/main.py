...
def cmd_providers(args):
    ...
    print(f"    openrouter  Nemotron 3 Ultra   https://openrouter.ai/keys")
    print(f"                export OPENROUTER_API_KEY=\"sk-or-...\"")
    print(f"                (free-tier model, independently rate-limited")
    print(f"                 from Gemini/Groq — a genuinely separate fallback)")
    print()
    ...
    # ── Sandbox isolation diagnostic ──────────────────────────────────────
    print(f"{'=' * 66}")
    print(f"  AGENTGUARD SANDBOX ISOLATION")
    print(f"{'=' * 66}")
    import subprocess as _sp
    try:
        v = _sp.run(["docker", "--version"], capture_output=True, timeout=5)
        binary_ok = (v.returncode == 0)
    except (FileNotFoundError, _sp.TimeoutExpired):
        binary_ok = False

    if not binary_ok:
        print("  Docker binary:     NOT FOUND")
        print("  Result:            Every exploit will run in the subprocess")
        print("                     fallback — OS resource limits only, no")
        print("                     filesystem or network isolation.")
    else:
        try:
            p = _sp.run(["docker", "ps"], capture_output=True, timeout=5)
            daemon_ok = (p.returncode == 0)
        except _sp.TimeoutExpired:
            daemon_ok = False
        print(f"  Docker binary:     found ({v.stdout.decode().strip()})")
        if daemon_ok:
            print("  Docker daemon:     reachable (docker ps succeeded)")
            print("  Result:            AgentGuard will use the Docker-isolated")
            print("                     path — container isolation, no network,")
            print("                     no host filesystem access beyond the")
            print("                     explicit mount.")
        else:
            print("  Docker daemon:     NOT reachable (docker ps failed)")
            stderr_tail = p.stderr.decode().strip()[:200] if p.stderr else "(no error output)"
            print(f"                     {stderr_tail}")
            print("  Result:            AgentGuard will silently fall back to the")
            print("                     subprocess sandbox for every exploit — the")
            print("                     Docker binary being installed is NOT enough")
            print("                     on its own. Common causes: the Docker daemon")
            print("                     is not running (try: sudo systemctl start docker),")
            print("                     or your user is not in the docker group (try:")
            print("                     sudo usermod -aG docker $USER, then log out")
            print("                     and back in).")
    print()
